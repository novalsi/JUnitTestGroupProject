...
public class TestDB {
    ...
    } //main  remove this after initial set up 
   //     System.out.println("*************");
...
*/
    /*
      //DAMS .....
        db.initialLoad("DAMS");
        System.out.println("\nAll appointments");
        List<Object> objs = db.getData("Appointments", "");
        for (Object obj : objs){
            System.out.println(obj);
            System.out.println("");
        }
        ...
        System.out.println("************");
        Appointments newAppt = new Appointments("800",6,java.sql.Date.valueOf("2009-09-01"),java.sql.Time.valueOf("10:15:00"));
        //extra steps here due to persistence api and join, need to create objects in list
        newAppt.setCustomervehicle((CustomerVehicle)db.getData("CustomerVehicle","customer='240' AND vehnum=1").get(0));
        newAppt.setTech((ServiceTech)db.getData("ServiceTech", "id='130'").get(0));
        List<Task> tasks = new ArrayList<Task>();
        tasks.add((Task)db.getData("Task", "id='900'").get(0));
        newAppt.setTaskCollection(tasks);
        List<Parts> parts = new ArrayList<Parts>();
        parts.add((Parts)db.getData("Parts", "id='9001'").get(0));
        newAppt.setPartsCollection(parts);

        boolean good = db.addData(newAppt);
        objs = db.getData("Appointments", "");
        for (Object obj : objs){
            System.out.println(obj);
            System.out.println("");
        }

        //use un-documented query method....
        String sql = "SELECT * from customer join appointments on Appointments.customer=Customer.id";
        ...
        for(HashMap<String,String> row: rs)
        {
            ...
            if (first)
            {
                ...
                for (Iterator it=row.keySet().iterator(); it.hasNext(); )
                {
                    ...
                    System.out.printf("%-25s",colName);
                }
                System.out.printf("%n");
                for (Iterator it=row.keySet().iterator(); it.hasNext(); )
                {
                    String colName = (String)it.next();
                    System.out.printf("%-25s",getUnderline(colName));
                }
                System.out.printf("%n");
            }
